...
public class PostgreSQLStorage implements ILogStorage
{
    ...
    private void flushBuffer() throws SQLException, IOException
    {
        synchronized(copyBuffer)
        {
            ...
            if ( LOG.isDebugEnabled() ) {
                LOG.debug("flushBuffer(): Flushing buffer ("+ copyBuffer.length()+" characters)");
            }
    ...
    private void flush(EternalThread.Context ctx) throws SQLException, IOException
    {
        while ( ! ctx.isCancelled() )
        {
            ...
            {
                if ( LOG.isDebugEnabled() ) {
                    LOG.debug("internalRun(): Flushing (timeout of "+config.flushInterval+" exceeded)");
                }
                doFlush = true;
            }
            else
            {
                ...
                if ( charsInBuffer > config.maxCharsInBuffer )
                {
                    if ( LOG.isDebugEnabled() )
                    {
                        LOG.debug( "internalRun(): Flushing (batch size " + config.maxCharsInBuffer + " exceeded)" );
                    }
                    doFlush = true;
                }
            }
